...
- (void)animateView:(BOOL)isShow textField:(id)textField heightforkeyboard:(CGFloat)kheight {
	...
	if (isShow) {
		if ([textField isKindOfClass:[UITextField class]]) {
            ...
            CGPoint textPoint1 = [newView convertPoint:CGPointMake(0, newView.frame.origin.y + spacerY) toView:objectView];
			if (rect.size.height - textPoint.y < kheight)
            {
				rect.origin.y = rect.size.height - textPoint.y - kheight + viewFrameY;
                if (rect.origin.y + textPoint1.y < 0) {
                    rect.origin.y = -textPoint1.y + 21;
                }
            }
			else rect.origin.y = viewFrameY;
		}
...
//输入框获得焦点
- (id)firstResponder:(UIView *)navView {
	for (UIView *aview in [self allSubviews:objectView]) {
        if (aview.hidden)
            continue;
...
//    MyLog(@"%@------%@", textView.text, text);
//	if ([[[text stringByReplacingOccurrencesOfString:@"\n" withString:@""] stringByReplacingOccurrencesOfString:@" " withString:@""] isEqualToString:@""] && [[[textView.text stringByReplacingOccurrencesOfString:@"\n" withString:@""] stringByReplacingOccurrencesOfString:@" " withString:@""] isEqualToString:@""]) {
...
- (void)textViewDidEndEditing:(MyTextView *)textView {
...
//            continue;
		if ([aview isKindOfClass:[UITextField class]]) {
			((UITextField *)aview).inputAccessoryView = aview.hidden ? nil : keyboardToolbar;
			((UITextField *)aview).delegate = self;
		}
		else if ([aview isKindOfClass:[UITextView class]]) {
			((UITextView *)aview).inputAccessoryView = aview.hidden ? nil : keyboardToolbar;
